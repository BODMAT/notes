...
const calendar: HTMLElement = document.querySelector(".task__calendar");
calendar.addEventListener("click", event => {
    updateCalendar(new Date().getMonth(), new Date().getFullYear());
});

function updateCalendar(rightMonth, rightYear): void {
    const dateInfoElements: NodeList = document.querySelectorAll(".task__date-info");
    ...
    const formattedDate: string = monthNames[rightMonth] + " " + currentYear;
    document.querySelector(".popup__month-year").textContent = formattedDate;

    //выставляю нужное количество дней для каждого месяца

    //31
    const dateTable: HTMLTableElement = document.querySelector(".popup__days");
    ...
    dateInfoArray.forEach((dateEl: HTMLDivElement) => {
    ...
    const btnPrev: HTMLButtonElement = document.querySelector(".popup__prev");
    const btnNext: HTMLButtonElement = document.querySelector(".popup__next");
...
function changeCalendarToPrev(month: number, year: number): void {
...
function changeCalendarToNext(month: number, year: number): void {
    let currentMonth: number = month;
    let currentYear: number = year;
    if (month === 11) {
        currentMonth = 0;
